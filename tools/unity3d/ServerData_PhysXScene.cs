...
partial class ServerData
{
    ...
    public static void exportPhysXScene()
    {
        ...
        foreach (var sphere in Object.FindObjectsOfType<SphereCollider>())
        {
            if (!m_excludes.Contains(sphere))
            {
                objs.Add(new PxSphereCollider(sphere));
            }
        }
    ...
    class PxSphereCollider : PxSceneObject
    {
        public static readonly PxObjectType clsType = PxObjectType.kSphereCollider;
        public override PxObjectType type { get { return clsType; } }
        public float radius;

        public PxSphereCollider() { }

        public PxSphereCollider(SphereCollider source)
        {
            position = source.transform.TransformPoint(source.center);
            rotation = source.transform.rotation;
            layer = (byte)source.transform.gameObject.layer;
